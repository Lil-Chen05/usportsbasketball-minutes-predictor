# Author: Model Visualization for Canadian University Basketball Data (Fixed)
# Date: 2024-12-19

"""
This script creates visualizations to compare model performance and show prediction accuracy.
It generates scatter plots, residual plots, and performance comparisons for Linear Regression and Random Forest.

Usage: 05-model_visualization_canadian_fixed.py --file_name=<file_name> --save_folder=<save_folder>
...
Example: python scripts/05-model_visualization_canadian_fixed.py --file_name=player_data_ready_canadian.csv --save_folder=results
"""
...
def create_performance_comparison(results, save_folder):
	...
	fig.suptitle('Model Performance Comparison (Canadian University Basketball - Fixed)', fontsize=16)
	...
	plt.tight_layout()
	plt.savefig(f'{save_folder}/model_performance_comparison_canadian_fixed.png', dpi=300, bbox_inches='tight')
	plt.close()
	
	# Save results to CSV
	df_results.to_csv(f'{save_folder}/model_performance_detailed_canadian_fixed.csv')
...
def create_prediction_scatter_plots(y_test, predictions, save_folder):
	...
	fig.suptitle('Predicted vs Actual Minutes Played (Canadian University Basketball - Fixed)', fontsize=16)
	...
	plt.tight_layout()
	plt.savefig(f'{save_folder}/prediction_scatter_plots_canadian_fixed.png', dpi=300, bbox_inches='tight')
	plt.close()
...
def create_residual_plots(y_test, predictions, save_folder):
	...
	fig, axes = plt.subplots(1, 2, figsize=(12, 6))
	fig.suptitle('Residual Plots (Canadian University Basketball - Fixed)', fontsize=16)
	...
	plt.tight_layout()
	plt.savefig(f'{save_folder}/residual_plots_canadian_fixed.png', dpi=300, bbox_inches='tight')
	plt.close()
...
def create_feature_importance_comparison(models, X, save_folder):
	...
	fig.suptitle('Feature Importance Comparison (Canadian University Basketball - Fixed)', fontsize=16)
	...
	plt.tight_layout()
	plt.savefig(f'{save_folder}/feature_importance_comparison_canadian_fixed.png', dpi=300, bbox_inches='tight')
	plt.close()
